...
class mf_widget extends WP_Widget
{
    ...
    private function _inject($widget) {
        ...
        $datablocksCurrent = get_option('mfdb_widget_options')['datablocks_url_option'];

        // If options are not set use default URLs
        $loaderURL = !empty($loaderURLCurrent) ? $loaderURLCurrent : DATABLOCKS_DEFAULT_LOADER_URL;
        $datablocksURL = !empty($datablocksCurrent) ? $datablocksCurrent : DATABLOCKS_DEFAULT_URL;
...
register_deactivation_hook( __FILE__, 'deactivate_mf_widget' );
